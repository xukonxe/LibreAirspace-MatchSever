...
namespace kcp2k
{
    public static class Common
    {
        ...
        public static void ConfigureSocketBuffers(Socket socket, int recvBufferSize, int sendBufferSize)
        {
            ...
            Log.Info($"[KCP] 接收缓冲区大小: {socket.ReceiveBufferSize} ({initialReceive}×{socket.ReceiveBufferSize/initialReceive})");
            Log.Info($"[KCP] 发送缓冲区大小: {socket.SendBufferSize} ({initialSend}×{socket.SendBufferSize/initialSend})");
        }
